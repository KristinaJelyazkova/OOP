...
private:
	...
	DynArray();
	//çà ñïåöèàëíè ñëó÷àè: íàïðèìåð, çà ñúçäàâàíå íà ìàñèâ îò ìàñèâè
	//Òîâà å âúçìîæíî ñàìî â ìåòîäè íà êëàñà
	//â.æ. çàäà÷àòà çà "slice" â äîìàøíîòî
	//âúïðîñ: êàêúâ å ïðîáëåìúò íà slice, àêî òîçè
	//êîíñòðóêòîð íå ñúùåñòâóâà?
...
{
	//êàê äà ïðåîäîëååì òîâà array = other.array?;
	size = other.size;
	array = new T[size];
...
void DynArray<T>::remove(const T& x)
{
	...
	size_t i;
	for (i = 0; i < index; i++)//íàëÿâî îò åëåìåíòà çà èçòðèâàíå
	{
		newarray[i] = array[i];
	}

	for (i = index + 1; i < size; i++)//íàäÿñíî îò åëåìåíòà çà èçòðèâàíå
	{
